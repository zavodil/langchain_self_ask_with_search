...
def create_agent(llm: BaseLanguageModel, tools: List[BaseTool], config: Dict[str, Any]):
    ...
    try:
        ...
        logger.warning(f"Error pulling prompt from hub: {e}")
        # Create a basic self-ask prompt if hub pull fails
...
Are follow up questions needed here:{agent_scratchpad}"""
    ...
    agent_executor = AgentExecutor(
        agent=agent,
        tools=tools,
        verbose=config.get("agent_config", {}).get("verbose", True),
        max_iterations=config.get("agent_config", {}).get("max_iterations", 10),
        early_stopping_method=config.get("agent_config", {}).get("early_stopping_method", "generate")
    )
