...
def initialize_tools(config: Dict[str, Any], llm: BaseLanguageModel) -> List[BaseTool]:
    ...
    try:
        ...
        intermediate_answer_tool = Tool(
            ...
            description="Search the web for intermediate answers to help answer the main question",
            func=search.run
        )
        ...
        intermediate_answer_tool = Tool(
            name="Intermediate Answer",
            description="Fallback tool when search is not available",
...
def save_output(result: Any, output_path: str, config: Dict[str, Any], error: str = None):
    ...
    if error:
        ...
        if isinstance(result, dict):
            ...
            output_data["result"] = {"output": str(result)}
...
def main():
    ...
    try:
        ...
        if len(tools) != 1:
            logger.warning(f"Self-ask agent requires exactly 1 tool, but {len(tools)} were initialized")
